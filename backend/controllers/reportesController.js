...
const getReporteInfraccionesPorFecha = async (req, res) => {
    try {
        ...
        switch (agrupacion) {
            case 'mes':
                groupByClause = "strftime('%Y', fecha_infraccion), strftime('%m', fecha_infraccion)";
                dateFormat = "strftime('%Y-%m', fecha_infraccion)";
                break;
            case 'semana':
                groupByClause = "strftime('%Y', fecha_infraccion), strftime('%W', fecha_infraccion)";
                dateFormat = "strftime('%Y-%W', fecha_infraccion)";
                break;
            default: // día
    ...
    try {
        ...
        const [infraccionesPorMes] = await pool.execute(`
            SELECT 
                ...
                COUNT(*) as cantidad
            FROM infracciones 
            WHERE fecha_infraccion >= datetime('now', '-6 months', 'localtime')
