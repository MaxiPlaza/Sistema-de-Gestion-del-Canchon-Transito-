...
const getDashboardStats = async (req, res) => {
    try {
        ...
        // Estadísticas básicas - CORREGIDO con estados correctos
        const [stats] = await pool.execute(`
            SELECT 
                ...
                (SELECT COUNT(*) FROM infracciones WHERE estado = 'activa') as infracciones_activas,
                (SELECT COUNT(*) FROM infracciones WHERE estado = 'resuelta') as infracciones_resueltas,
                (SELECT COUNT(*) FROM infracciones WHERE estado = 'anulada') as infracciones_anuladas,
                (SELECT COUNT(*) FROM usuarios WHERE activo = true) as total_usuarios,
                (SELECT COUNT(*) FROM infracciones WHERE DATE(fecha_infraccion) = date('now', 'localtime')) as infracciones_hoy
        `);

        console.log('📈 Estadísticas básicas:', stats[0]);

        // Infracciones por tipo de vehículo (últimos 30 días)
        const [infraccionesPorTipo] = await pool.execute(`
            SELECT 
                COALESCE(v.tipo_vehiculo, 'No especificado') as tipo_vehiculo,
                COUNT(i.id) as cantidad
            FROM infracciones i
            LEFT JOIN vehiculos v ON i.vehiculo_id = v.id
            WHERE i.fecha_infraccion >= datetime('now', '-30 days', 'localtime')
        ...
        const [infraccionesPorDia] = await pool.execute(`
            SELECT 
                ...
                COUNT(*) as cantidad
            FROM infracciones 
            WHERE fecha_infraccion >= datetime('now', '-7 days', 'localtime')
        ...
        // Infracciones por estado - CORREGIDO con estados correctos
        ...
        console.log('📊 Infracciones por estado:', infraccionesPorEstado);
